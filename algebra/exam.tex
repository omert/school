...
%    polynomial of $a$. 
  ...
  \item
    ...
    $p$ is prime, can only be $C_{p^2}$ or $C_p \times C_p$. In order to solve this problem,
    we first prove a similar and more general claim:
    \begin{proposition}
      \label{eq:byprime}
      Let $p$ and $q$ be primes, and let $G$ be a group of order $pq$. Then
      $G$ is isomorphic to $C_{pq}$ or $G$ is isomorphic to $C_p \times C_q$.
    \end{proposition}
    \begin{proof}
      Clearly this follows if we show that $G$ is Abelian. Assume then by
      contradiction that $G$ is not Abelian, and let $a,b \in G$ be a pair
      of elements that don't commute. Since we assumed $G$ is not Abelian then 
      it is not cyclic, and so the orders of $a$ and $b$ are less than $pq$. 
      Since they don't commute then neither is the unit, and so their orders
      are greater than one. Without loss of generality let the order of $a$
      be $p$. Then the centralizer of $a$, $C_G(a)$ is of size $p$, too, since
      it includes the group generated by $a$ and so has to be a multiple of $p$,
      and doesn't include $b$ and so cannot equal $pq$. 
