  ...
  \item
    ...
    \item
      ...
      \item
        ...
        Now 
        \begin{eqnarray*}
          \E[D_{n\log n(1+\epsilon)}] &=& \sum_{i=0}^n\P[D_{n\log n(1+\epsilon)}=i]i
          \\ &\geq& \sum_{i=1}^n\P[D_{n\log n(1+\epsilon)}=i]
          \\ &=& 1-\P[D_{n\log n(1+\epsilon)}=0],
        \end{eqnarray*}
        and so since~\eqref{eq:plus_eps} implies that $\E[D_{n\log n(1+\epsilon)}]$
        goes to zero as $n$ goes to infinity, then $\P[D_{n\log n(1+\epsilon)}=0]$
        goes to one. Hence with probability that approaches one, after
