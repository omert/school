...
a parallel coordinate system. We choose two intersecting (straight) lines 
...
one and only one ${\bf x}$. Following Euler \cite{Euler:1749} and 
...
The points ${\bf x}$ of a line $\mathfrak{g}$ are mapped by an affine 
transformation to
the points ${\bf x}^*$ of a line $\mathfrak{g}^*$. In fact, a line 
$\mathfrak{g}$ can be
...
satisfied, and thus again the locus of ${\bf x}^*$ is a line.

It further follows from~\ref{eq:affine_trans} that affine transformations are
continuous mappings. That is, as ${\bf x}$ tends to ${\bf x}_0$ the 
corresponding ${\bf x}^*$ tends to ${\bf x}^*_0$.

With help from M\"obius nets ~\cite{Mobius:1827} one can show that the last
...
are characterized by not having common points.
