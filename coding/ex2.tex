...
Let $P_{m,d}(X_1,\ldots,X_m)$ be the 
encoding polynomial for $m$ variables with max total degree $d$. 
The degree of the difference
between two different such polynomials, $P_{m,d}$ and $P'_{m,d}$, is at most $d$, and
therefore has at most $dq^{m-1}$ zeros: fix the first $m-1$ variables to
arrive at a polynomial of degree $d$ in the last variable. This polynomial
has at most $d$ zeros. Now repeat for the $q^{m-1}$ possible
values of the first $m-1$ variables. 
